...
// We run the schema as individual statements so we can handle each gracefully
const statements = [
  // Fix existing users constraint if present
  `ALTER TABLE IF EXISTS users DROP CONSTRAINT IF EXISTS users_role_check`,
  `ALTER TABLE IF EXISTS users ADD CONSTRAINT IF NOT EXISTS users_role_check 
   CHECK (role IN ('Admin','Procurement Officer','Finance Manager','Vendor','Manager','Approver'))`,
  ...
  try {
    for (const sql of statements) {
      try {
        ...
        // Warn but continue — some ALTER TABLE may fail if table doesn't exist yet
